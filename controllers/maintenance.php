...
function maintenance_mode_block_if_enabled(): void {
    ...
    $target = $_SERVER['HTTP_REFERER'] ?? '/redmine/?page=dashboard';
...
function maintenance_import_bundle(array $bundle, array $selected): int {
    if (!in_array(($bundle['type'] ?? ''), ['redmine-maintenance', 'redmine-mantencion-maintenance'], true) || !isset($bundle['sections']) || !is_array($bundle['sections'])) {
...
function maintenance_import_package(string $uploadedFile, array $selected): int {
    ...
    if (!is_array($manifest) || !in_array(($manifest['type'] ?? ''), ['redmine-maintenance-package', 'redmine-mantencion-maintenance-package'], true)) {
        maintenance_remove_dir($extractDir);
...
function maintenance_redirect_back(): void {
    header('Location: /redmine/?page=configuracion');
    exit;
}
